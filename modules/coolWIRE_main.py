...
# ============================================================
# ============================================================
# TAB 5 – ROHRNETZ (coolROHR Integration)
# ============================================================
with t5_rohr:
    from modules.rohrnetz import (
        berechne_leitung, CU_PIPES, REFRIGERANTS,
        calc_pipe, select_pipe, dp_limit_K, equiv_length,
        hydrostatic_dp, insulation_thickness_mm, darcy_f
    )

    st.markdown('<div class="sec">╔ Kältetechnische Rohrnetzberechnung – coolROHR v6.2</div>', unsafe_allow_html=True)
    ...
    if not ks_rohr:
        ...
        # Kältemittel & Kondensationstemperatur je Kreis
        # (aus Maschinenstandorten oder Defaults)
        KREIS_AU = {
            1: {"modell":"MDV-SY-50582 #2", "ref":"R513A", "t0_c":-8,  "tc_c":47, "app":"NK"},
            2: {"modell":"Sigilus BDF-NG-1075","ref":"R449A","t0_c":-28,"tc_c":42, "app":"TK"},
            3: {"modell":"MDV-SY-50582 #1", "ref":"R513A", "t0_c":-8,  "tc_c":47, "app":"NK"},
        }

        # Session State für Rohrnetz-Overrides
        if "rn_offsets" not in st.session_state:
            st.session_state.rn_offsets = {}
        if "rn_params"  not in st.session_state:
            st.session_state.rn_params  = {}

        def v_bar(v, vmin, vmax):
            scale = max(vmax * 1.3, v * 1.1, 1)
            pct = min(v / scale * 100, 100)
            pm  = vmin / scale * 100
            px  = vmax / scale * 100
            col = "#4CAF50" if vmin <= v <= vmax else "#f44336"
            st  = "✅" if vmin <= v <= vmax else ("⬇️" if v < vmin else "⬆️")
            return (f"<div style='margin:4px 0'>"
                    f"<div style='display:flex;justify-content:space-between;font-size:11px'>"
                    f"<span>v = <b>{v:.2f} m/s</b></span><span style='color:{col}'>{st}</span></div>"
                    f"<div style='position:relative;height:14px;background:#e0e0e0;border-radius:7px;overflow:visible'>"
                    f"<div style='position:absolute;left:0;top:0;height:100%;width:{pct:.1f}%;background:{col};border-radius:7px'></div>"
                    f"<div style='position:absolute;left:{pm:.1f}%;top:-3px;height:20px;width:2px;background:#1565C0'></div>"
                    f"<div style='position:absolute;left:{px:.1f}%;top:-3px;height:20px;width:2px;background:#b71c1c'></div>"
                    f"</div><div style='display:flex;justify-content:space-between;font-size:9px;color:#666'>"
                    f"<span>min {vmin:.1f}</span><span>max {vmax:.1f} m/s</span></div></div>")

        def dp_bar(dp_K, dp_max, dp_mbar):
            pct = min(dp_K / max(dp_max,0.01) * 100, 130)
            col = "#4CAF50" if dp_K <= dp_max else "#f44336"
            return (f"<div style='margin:4px 0'>"
                    f"<div style='display:flex;justify-content:space-between;font-size:11px'>"
                    f"<span>Δp = <b>{dp_K:.3f} K</b> | {dp_mbar:.1f} mbar</span>"
                    f"<span style='color:{col}'>{'✅' if dp_K<=dp_max else '⚠️'} Grenz {dp_max:.1f} K</span></div>"
                    f"<div style='height:10px;background:#e0e0e0;border-radius:5px;overflow:hidden'>"
                    f"<div style='height:100%;width:{min(pct,100):.1f}%;background:{col};border-radius:5px'></div>"
                    f"</div></div>")

        for kr_nr in sorted(kreise_rohr.keys()):
            ks_liste = kreise_rohr[kr_nr]
            au       = KREIS_AU.get(kr_nr, {"modell":"?","ref":"R513A","t0_c":-8,"tc_c":47,"app":"NK"})
            kw_sum   = round(sum(k.get("kaelteleistung_kw",0) for k in ks_liste), 2)
            ref_key  = au["ref"]
            ref_name = REFRIGERANTS[ref_key]["name"]
            app_code = au["app"]

            # Längste Leitung als Hauptstrang
            max_L = max((k.get("leitungslaenge_m",0) for k in ks_liste), default=10)

            exp_label = (f"Kreis {kr_nr} – {au['modell']} · {ref_name} · "
                         f"{kw_sum} kW · {len(ks_liste)} KS · t₀={au['t0_c']}°C")
            with st.expander(exp_label, expanded=True):

                # ── Kopf-Metriken
                mc1,mc2,mc3,mc4,mc5 = st.columns(5)
                mc1.metric("Kältemittel", ref_name)
                mc2.metric("Verdampfung t₀", f"{au['t0_c']}°C")
                mc3.metric("Kondensation tc", f"{au['tc_c']}°C")
                mc4.metric("Gesamtleistung", f"{kw_sum} kW")
                mc5.metric("Kühlstellen", len(ks_liste))
                ...
                for ks in ks_liste:
                    rohr_data.append({
                        ...
                        "kW": ks.get("kaelteleistung_kw","?"),
                        "L [m]": ks.get("leitungslaenge_m","?"),
                        "Raum [°C]": ks.get("raum_temp_soll_c","?"),
                        "Verdampfung [°C]": ks.get("verdampfung_custom_c", au["t0_c"]),
                    })
                st.dataframe(pd.DataFrame(rohr_data), use_container_width=True, hide_index=True)

                st.markdown("---")
                st.markdown("**⚙️ Rohrleitungsparameter**")

                # ── Parameter-Eingaben
                pk = f"rn_{kr_nr}"
                if pk not in st.session_state.rn_params:
                    st.session_state.rn_params[pk] = {
                        "L_h": float(max_L), "L_v": 3.0,
                        "n_el": 4, "n_bv": 1, "n_sv": 1,
                        "h_SL": 3.0, "h_FL": -3.0,
                        "T_amb": 25.0, "phi": 70.0,
                    }
                prm = st.session_state.rn_params[pk]

                pc1,pc2,pc3 = st.columns(3)
                with pc1:
                    prm["L_h"] = st.number_input("SL horizontal [m]", 0.0, 500.0, prm["L_h"], 1.0, key=f"rn_lh_{kr_nr}")
                    prm["L_v"] = st.number_input("SL vertikal [m]",   0.0, 100.0, prm["L_v"], 0.5, key=f"rn_lv_{kr_nr}")
                with pc2:
                    prm["n_el"] = st.number_input("Bögen",        0, 30, prm["n_el"], 1, key=f"rn_el_{kr_nr}")
                    prm["n_bv"] = st.number_input("Kugelhähne",   0, 20, prm["n_bv"], 1, key=f"rn_bv_{kr_nr}")
                    prm["n_sv"] = st.number_input("Magnetventile",0, 10, prm["n_sv"], 1, key=f"rn_sv_{kr_nr}")
                with pc3:
                    prm["h_SL"]  = st.number_input("Höhe SL [m]", -30.0, 30.0, prm["h_SL"],  0.5, key=f"rn_hsl_{kr_nr}")
                    prm["h_FL"]  = st.number_input("Höhe FL [m]", -30.0, 30.0, prm["h_FL"],  0.5, key=f"rn_hfl_{kr_nr}")
                    prm["T_amb"] = st.number_input("T_amb [°C]",  10.0,  45.0, prm["T_amb"], 1.0, key=f"rn_ta_{kr_nr}")
                    prm["phi"]   = st.number_input("Feuchte [%]", 30.0, 100.0, prm["phi"],   5.0, key=f"rn_ph_{kr_nr}")

                if st.button(f"⚡ Kreis {kr_nr} berechnen", key=f"rn_calc_{kr_nr}", type="primary"):
                    st.session_state.rn_params[pk]["_calc"] = True
                    st.session_state.rn_params[pk]["_Q"]    = kw_sum
                    st.rerun()

                if st.session_state.rn_params[pk].get("_calc"):
                    Q   = st.session_state.rn_params[pk]["_Q"]
                    res = berechne_leitung(
                        ref_key, au["t0_c"], au["tc_c"], Q,
                        prm["L_h"], prm["L_v"],
                        int(prm["n_el"]), int(prm["n_bv"]), int(prm["n_sv"]),
                        prm["h_SL"], prm["h_FL"],
                        prm["T_amb"], prm["phi"], app_code
                    )

                    st.markdown("---")
                    st.markdown(f"**Ergebnis – Massenstrom ṁ = {res['m_dot_kgh']} kg/h · p₀ = {res['p0_bar']} bar · pc = {res['pc_bar']} bar**")

                    r1,r2,r3 = st.columns(3)
                    for col_ui, line_key, title, color in [
                        (r1,"SL","🔵 Saugleitung","#1565C0"),
                        (r2,"DL","🔴 Druckleitung","#b71c1c"),
                        (r3,"FL","🟢 Flüssigkeitsleitung","#2E7D32"),
                    ]:
                        ln = res[line_key]
                        pipe = ln["pipe"]
                        with col_ui:
                            st.markdown(f"**{title}**")
                            st.markdown(f"<div style='text-align:center;font-size:22px;font-weight:bold;color:{color}'>⌀ {pipe['od']:.0f} / {pipe['id']:.0f} mm</div>", unsafe_allow_html=True)
                            st.caption(f"{pipe['label']} mm · L_eq = {ln['L_eq']:.1f} m")
                            st.markdown(v_bar(ln["v"], ln["vmin"], ln["vmax"]), unsafe_allow_html=True)
                            st.markdown(dp_bar(ln["dp_K"], ln["dp_max_K"], ln["dp_bar"]*1000), unsafe_allow_html=True)
                            st.caption(f"Re = {ln['Re']:,.0f} · Isolierung: {ln['insul_mm']} mm")
                            for w in ln.get("warns",[]):
                                st.warning(w)

                    # SL v_min Steigleitung Check
                    from modules.rohrnetz import get_sat_props
                    props0 = get_sat_props(ref_key, au["t0_c"])
                    vmin_sl_v = 9.0 if app_code=="TK" else 7.6
                    if prm["L_v"] > 0.5 and res["SL"]["v"] < vmin_sl_v:
                        st.warning(f"⚠️ v = {res['SL']['v']:.1f} m/s < v_min Steigleitung {vmin_sl_v} m/s → **Doppelsteigrohr (DSR) empfohlen!**")
